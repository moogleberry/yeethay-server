...
MongoClient.connect('mongodb://localhost:27017/yeethay', function(err, client) {
	...
	app.get("/rest/ratingOptions", (req, res) => {
		db.collection('rating_options').find({}, {
			"sort": {
				"value": 1
			}
	...
	app.get("/rest/getItem", (req, res) => {
		db.collection('rating_items').find(req.query, {
			"limit": 1
	...
	app.get("/rest/getItems", (req, res) => {
		db.collection('rating_items').find(req.query).toArray(function (err, result) {
			if (err) throw err;
			res.send(result);
		});
	});
	...
	app.get("/rest/unratedItem", (req, res) => {
		db.collection('rating_items').find({
			"name": "Lychee"
		}, {
			"limit": 1
		}).next(function (err, result) {
